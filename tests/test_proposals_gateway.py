...
def test_submit_proposal_ids_are_unique(gateway, seth):
    a = gateway.submit_proposal("foo.json", "{}", "first", seth)
    b = gateway.submit_proposal("foo.json", "{}", "second", seth)
...
def test_submit_proposal_preserves_content_verbatim(gateway, remote, seth):
    ...
    status = gateway.submit_proposal("vehicle_layouts.json", new_content, "test", seth)
...
def test_list_my_proposals_filters_to_current_user(gateway, seth, chris):
    a = gateway.submit_proposal("parts_library.json", "{}", "seth one", seth)
    b = gateway.submit_proposal("build_rules.json", "{}", "seth two", seth)
    gateway.submit_proposal("parts_library.json", "{}", "chris one", chris)
...
def test_list_my_proposals_skips_malformed_files(gateway, remote, seth, caplog):
    ...
    good = gateway.submit_proposal("foo.json", "{}", "good one", seth)
...
def test_list_my_proposals_skips_non_json_entries(gateway, remote, seth):
    ...
    submitted = gateway.submit_proposal("foo.json", "{}", "real", seth)
    out = gateway.list_my_proposals(seth)
...
def test_list_my_proposals_handles_pickup_race(gateway, remote, seth):
    ...
    submitted = gateway.submit_proposal("foo.json", "{}", "race", seth)
