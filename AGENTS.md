...
CLAUDE.md                          ← same brief, Claude-specific tooling notes
...
src/dtm_buildsheet/                ← the Python package
  ...
  resources/
    ...
    presets/*.json                 ← bundled default presets (in dev, workspace writes here too)
...
workspace/                         ← mutable user data (git-ignored)
  ...
  presets/   ← user-created/imported presets (bundled app only; dev writes to resources/presets/)
...
#proj-build-editor   — embedded build editor (replaces switching away to generate tab)
...
`#card-preview` and `#card-manifest` live exclusively inside `#proj-build-editor`. They are not duplicated elsewhere.

### Settings tab
Ten sub-tabs (stabs):
`placements | fixtures | sizes | catalog | parts | vehicles | agencies | sales-reps | presets | tools`

The **Tools** stab contains the standalone build-sheet generator (generate workbook from an `.xlsx` upload), formerly the main "Generate" tab.
...
Agency search uses `difflib.get_close_matches` after normalizing common abbreviations (PD→police department, SO→sheriff's office, St.→saint, etc.).

## Preset system

Presets are JSON files (schema_version 2) with fields:
```json
{
  "preset_id": "...",
  "label": "St. Cloud PD Patrol PIU/Tahoe",   // auto-generated from agency + build_type + vehicles
...
- **Dev mode**: `workspace/presets/` points to `src/dtm_buildsheet/resources/presets/` so new presets are tracked by git and shipped to users.
- **Bundled app**: workspace presets in `~/Library/Application Support/.../presets/` (user-specific).
...
`app/services/config_service.py` defines `TEMPLATE_REGEN_FILES`. Whenever `save_config_file()` is called for one of those files, it triggers template regeneration as a background thread side effect. If you add a new config file that feeds `template_builder.py`, add its filename to `TEMPLATE_REGEN_FILES`.

## Route module pattern

Every route module: `route_xxx(handler, method, path, body, paths) → bool`. Returns `True` if handled.
...
## Gotchas
- **Python package name** is `dtm_buildsheet` / `dtm-buildsheet` — only the *app* name is "DTM Vehicle Builder". Don't rename the `src/dtm_buildsheet/` directory without updating all imports.
...
- **#card-preview and #card-manifest are singletons** — they exist only inside `#proj-build-editor`. The standalone generate tool in Settings → Tools does not show them.
- **Modal ownership** — each modal's save button is owned by exactly one IIFE. Never attach a second listener from another file.
