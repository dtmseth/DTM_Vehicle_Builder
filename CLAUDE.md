...
`AgencyRecord` carries an optional `qb_customer_id` (FK → QuickBooks `Customer.Id`). The QuickBooks down-sync (`qb_sync_service.import_customers` → `agency_service.upsert_agencies_from_qb`) pulls top-level QB Customers into agencies, matching by `qb_customer_id` then normalized name, filling only empty contact fields (never clobbering user data). Bulk imports propagate via `save_settings_to_cloud_batch_in_background` (one thread, no per-record audit proposals) — do NOT swap this back to per-record `save_setting_to_cloud_in_background` in a loop, or a first import spawns hundreds of threads. Agency→QB **up-sync** is built: `handle_save_agency` fires `qb_sync_service.push_agency_in_background` after the SharePoint mirror, which creates/sparse-updates the QB Customer (Customer is the integration's only write target — never financial entities) and writes `Customer.Id` back via `agency_service.set_qb_customer_id` (deliberately NOT through `handle_save_agency`, so the write-back can't re-trigger a push). The per-vehicle sub-customer/job bridge is built: `qb_sync_service.push_vehicle_job(project_id, individual_id)` ensures the agency Customer exists, then creates/reuses a QBO sub-customer (`Job=true` + `ParentRef`) per `IndividualUnit` — the per-vehicle "project" container (`Customer.IsProject` is read-only via the API; jobs convert to Projects in the QBO UI) — writing `qb_job_id` back. `qb_estimate_service` drafts **non-posting Estimates** per vehicle from the build's selected parts: `resolve_build_lines` maps each part to a QB-linked catalog product (`qb_item_id`/`qb_unit_price`), `validate_estimate` is an offline dry-run, and `create_estimate` BLOCKS unless every part is linked/active/priced (reason codes: `no_catalog_match`/`not_linked`/`qb_inactive`/`no_price`), ensures the job, then writes `qb_estimate_id` back. The app writes Customers + non-posting Estimates only — never Invoices or any posting transaction (estimate→invoice conversion stays an explicit user action in QBO). `IndividualUnit` carries `qb_job_id`/`qb_estimate_id`/`qb_invoice_id` (codec round-trips them). UI buttons on the Builds tab calling `/api/quickbooks/estimates/*` are not yet wired.
