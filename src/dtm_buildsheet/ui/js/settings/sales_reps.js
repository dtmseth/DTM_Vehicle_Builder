          ...
          ${list.map(r => `
            <tr style="border-bottom:1px solid var(--border)">
              ...
              <td style="padding:7px 8px;white-space:nowrap;text-align:right">
                <button class="btn btn-secondary btn-sm" onclick="salesRepEdit('${esc(r.rep_id)}')">Edit</button>
                <button class="btn btn-secondary btn-sm" style="margin-left:4px;color:var(--red)" onclick="salesRepDelete('${esc(r.rep_id)}','${esc(r.name)}')">Delete</button>
              </td>
            </tr>`).join("")}
        </tbody>
      </table>`;
