...
def route_presets(handler: BaseHTTPRequestHandler, method: str, path: str,
    ...
    if method == "GET" and path == "/api/presets":
        send_json(handler, {"ok": True, "presets": list_presets(paths)})
        return True
    ...
    if method == "GET" and path.endswith("/export-workbook"):
        ...
        if preset_id and "/" not in preset_id:
            try:
                ...
                send_json(handler, {"ok": False, "error": str(exc)})
            return True
    ...
    if method == "POST" and path == "/api/presets/save":
        ...
        send_json(handler, save_preset(body, paths, overwrite=overwrite))
        return True
    ...
    if method == "POST" and path == "/api/presets/import-workbook":
        ...
        if not file_b64:
            send_json(handler, {"ok": False, "error": "No file data provided"})
        else:
            send_json(handler, import_from_workbook(file_b64, paths))
        return True
    ...
    if method == "POST" and path.startswith("/api/presets/") and path.endswith("/clone"):
        ...
        if preset_id and "/" not in preset_id:
            send_json(handler, clone_preset(preset_id, paths))
            return True
    ...
    if method == "DELETE" and path.startswith("/api/presets/"):
        preset_id = path[len("/api/presets/"):]
        if preset_id and "/" not in preset_id:
            send_json(handler, delete_preset(preset_id, paths))
            return True
