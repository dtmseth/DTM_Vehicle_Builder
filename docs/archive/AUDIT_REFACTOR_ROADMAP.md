...
- **Committed future features the architecture must hold space for** (per `ROADMAP.md`
  near-term critical path, reconciled 2026-07-07 — the picker + placement cluster shipped
  2026-07-01, see Step 3): kit SKUs (NEXT, gated on the Step 4 extraction), QB Pass-2 import,
  finishing the Part Picker (Chunks 8–9 + non-light coverage), Phase 4
...
**Step 3 ⭐ — Picker + placement cluster** ✅ *shipped 2026-07-01 — before this plan existed*
(discovered during Step 4 design; PARTS_DB_AND_PICKER §"SHIPPED" is authoritative, ROADMAP.md's
2026-06-30 critical path had lagged — both reconciled 2026-07-07). Items #4/#7/#5/#8b landed
(commits `48e5634`, `b9022aa`, `1a7a79e` era), unpinned because the pins didn't exist yet.
**Consequences**: (a) the Step-1 pin implementations now serve the *Step 4 extraction* as their
first customer, not this cluster; (b) this step's smoke-suite done-check folds into the 1c
implementation session (the six flows exercise the shipped cluster and pin it retroactively);
(c) Step 4 Stage A is unblocked as soon as the Step 1 pins are implemented. Only cluster
leftover: #7(a) auto-skip empty location step (low priority, ROADMAP-tracked).
...
**Design ✅** *(this commit)*: interface + migration spec in
`docs/audit/PARTS_DB_REPOSITORY_SPEC.md`. Decisions: home is a new
`src/dtm_buildsheet/parts_db/` package (repository/schema/legacy/writer modules; dataclasses
stay in `domain/`); lint layers become `app → inputs → config → planning → parts_db →
storage → rules → domain` (verified feasible — nothing imports upward across the new
positions); reads move wholesale at extraction (mechanical), writes stay on
`save_config_file` until a later Stage C lands an explicit writer port (no silent local
default — the SP-revert footgun rules it out); kit-SKU composability reserved at the SKU
level (`schema.py` validation + `expand_kit()` query), nothing implemented. Inventory
findings absorbed: the 60s cloud sync never invalidates the parts_db read cache (stale-cache
window, fix scheduled as flagged improvement); domain dataclasses lack the shipped
`location_mode`/accessory-role/`reviewed` fields (routes compensate via `raw_doc()` — closing
this re-records the 1b contracts intentionally); GOTCHAS #21 is drift. Extraction runs after
Step 3 per ordering, Stage A alone meets the done-condition; three owner questions logged
(kit estimate behavior, sibling-app write mode, SKU-grid command-layer timing vs curation).
...
| 1.10 | 2026-07-07 | §8.1 Step 4 design SHIPPED — parts-DB repository interface + migration spec (`docs/audit/PARTS_DB_REPOSITORY_SPEC.md`). Grounded in a verified consumer inventory (planner lazy import uses only `list_part_types()`; qb_estimate/qb_sync/routes are the `raw_doc()` dippers; routes carry ~350 LOC of edit commands plus placement-judgment tables). Home: new `parts_db/` package below `planning` in a revised lint layering; config imports the parts_db validator downward so `save_config` semantics stay byte-identical. Round-trip invariant makes curation state safe by construction (writes never pass through dataclass hydration). Migration split into Stage A (repository extraction — deletes the planning→app baseline entry, satisfies the Step 4 done-condition), B (consumer repoint), C (writer port + command layer + hydration-gap close + sync-invalidation fix), each pinned to 1a/1b/1c artifacts. New findings: cloud-sync stale-cache gap (F-1), dataclass hydration gap vs shipped conventions (F-2), GOTCHAS #21 drift (F-4). Zero production-code changes; extraction session gated on Step 3 + recorded pins. |
| 1.11 | 2026-07-07 | Step 3 reconciled: the picker + placement cluster shipped 2026-07-01 — *before this plan existed* (PARTS_DB_AND_PICKER §"SHIPPED" authoritative; ROADMAP.md's 2026-06-30 critical path had lagged and is now corrected too, with kit SKUs promoted to ⭐ NEXT and explicitly gated on the Step 4 Stage A extraction). Consequences recorded in Step 3: pins' first customer is the Step 4 extraction; the 1c implementation pins the shipped cluster retroactively; Stage A unblocks once Step 1 pins are implemented. §0 future-features list updated. |
| 1.12 | 2026-07-07 | Parts-curation proposal pipeline SHIPPED on the data track (commits 9ab4e42 / 1e009bf / 270fe6a / d5a5b5a; zero parts_db writes, verified). Queue corrected 673→384 (high-confidence triage tier had already been applied); 384/384 proposal coverage (222 H / 139 M / 42 L, 72 named owner questions). Artifacts: PARTS_CURATION_AUDIT.md, workbook relationship graph + reconciliation (2 true conflicts), 12 curate.py plans + REVIEW_SHEET.md, PARTS_CLASSIFICATION_GUIDE.md (post-Fable continuity). Owner workflow: rule on scene-light collapse + audit §3 questions first, review sheet, apply plans in filename order (dev app closed, backup checkpoint first, commit per plan). Stage C2 gate unchanged: opens only when the owner declares the queue done. Workbook-graph gap closure doubles as the first pipeline-inversion parity evidence. |
