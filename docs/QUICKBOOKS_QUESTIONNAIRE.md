...
> The refresh token and realm ID are encrypted using AES-256-GCM before being written to a local configuration file (`quickbooks_config.json`). The AES encryption key is stored in a separate file (`quickbooks_key.bin`). Both files are restricted to the local machine, are excluded from version control, and are never synced to any external service. The client secret is also encrypted in the same configuration file.
...
> The refresh token, access token, realm ID, and client secret are all encrypted using AES-256-GCM before being written to disk. The encryption key is stored in a separate file from the encrypted values, as required. Plaintext token values are never written to disk, logged, or exposed in the application UI.
