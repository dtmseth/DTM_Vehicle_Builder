...
**Parts-import effort (in progress)**: folding the ~1,200-item QBO inventory CSV into the catalog incrementally. Pass 1 (manufacturers) done — `tools/qb_inventory_import.py` added 13 manufacturers (48→61) and emitted `tools/qb_category_to_manufacturer.json`. The owner chose to **seed the sandbox via API** (`tools/qb_seed_sandbox.py`) since the manual sandbox CSV import failed.

**Pass 2 (items) tooling built — 2026-06-26**: `tools/qb_inventory_import_pass2.py` is a *proposer, not a writer* — it never touches parts_db. Per manufacturer it emits a reviewable `qb_apply_links.py` mapping (`tools/qb_links/qb_inv_<mid>.json`) + a report. **Policy (locked w/ Seth):** mechanism = **pending-QB** (each item added as an orderable `part_number` with the CSV Price + `qb_pending=true`; reconciles to the QBO Item Id on sync — the CSV has no Item Id); granularity = **one product per SKU**, conservative attach (only attaches to an existing product on a confident model match, else new product with `fits_part_types: []` = catalog-only until a part type is assigned in the Part Manager); **skips** already-imported part#s + junk rows (implausible/placeholder part#, blank description, missing price) into a REVIEW list so nothing is silently created/dropped. Profile of the export: 1284 rows → 620 already in parts_db, 581 new, 83 unmapped (blank/MISC). **Pilot run (Gamber Johnson + Setina):** Gamber → 72 new products proposed (`tools/qb_links/qb_inv_gamber_johnson.json`, validates through `qb_apply_links.py` = 144 actions, **not yet applied** pending owner review); Setina → nothing to import (160 already in db, 3 junk rows flagged incl. part# `c` whose desc is really a `PB450L` push bumper — needs a real SKU). **Next:** owner reviews/apply the Gamber pilot, then run the proposer for the remaining manufacturers.
